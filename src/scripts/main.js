...
// Storage bridge: read/write chrome.storage.local so the MV3 service worker stays in sync.
function setLocal(items) { chrome.storage.local.set(items); }
function removeLocal(keys) { chrome.storage.local.remove(typeof keys === 'string' ? [keys] : keys); }

modHeader.factory('dataSource', function($mdToast) {
  var dataSource = {};

  // Cache activeTabId and currentTabUrl read from chrome.storage.local at startup.
  // (Background service worker writes these; popup localStorage can't see SW writes.)
  chrome.storage.local.get(['activeTabId', 'currentTabUrl'], function(data) {
    dataSource.activeTabId = data.activeTabId || null;
    dataSource.currentTabUrl = data.currentTabUrl || null;
  });
  // Keep cache fresh whenever background updates these.
  chrome.storage.onChanged.addListener(function(changes, area) {
    if (area !== 'local') return;
    if ('activeTabId' in changes) dataSource.activeTabId = changes.activeTabId.newValue;
    if ('currentTabUrl' in changes) dataSource.currentTabUrl = changes.currentTabUrl.newValue;
  });
  ...
  dataSource.addFilter = function(filters) {
    let urlRegex = '';
    const tabUrl = dataSource.currentTabUrl || localStorage.currentTabUrl;
    if (tabUrl) {
  ...
  dataSource.lockToTab = function() {
    dataSource.lockedTabId = dataSource.activeTabId || localStorage.activeTabId;
    localStorage.lockedTabId = dataSource.lockedTabId;
    setLocal({ lockedTabId: dataSource.lockedTabId });
    $mdToast.show(
      $mdToast.simple()
  ...
  dataSource.save = function() {
    ...
    // Sync to chrome.storage.local so the MV3 service worker picks up the change.
    setLocal({ profiles: serializedProfiles, selectedProfile: selectedProfileIndex });
  };
  return dataSource;
});
